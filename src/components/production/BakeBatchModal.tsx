...
export const BakeBatchModal: React.FC<BakeBatchModalProps> = ({ recipe, onClose }) => {
  const { products, produceBatch } = useApp();
  const [batchCount, setBatchCount] = useState<number>(1);
  const [poProduct, setPoProduct] = useState<Product | null>(null);

  // Compute live availability and shortfalls
        ...
        <div className="mt-5 space-y-5 text-xs">
          ...
          {hasShortage ? (
            ...
            <div className="p-3.5 rounded-2xl bg-emerald-50 border border-emerald-200 text-emerald-800 flex items-start space-x-2 text-xs">
              ...
              <div>
                <strong className="font-bold block">Feasibility Verified</strong>
                <span>
                  All {ingredientChecks.length} ingredients are available. Executing bake will deduct raw materials and reconcile with 30-day demand forecast.
                </span>
              </div>
            </div>
          )}
